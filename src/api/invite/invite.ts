...
export const claimInvite = (
    ...
    {url: `/api/v1/invites/${token}/claim`, method: 'POST'
    },
    );
  }
...
export const getClaimInviteMutationKey = (token: string,) => [`/api/v1/invites/${token}/claim`] as const;

export type ClaimInviteMutationResult = NonNullable<Awaited<ReturnType<typeof claimInvite>>>
export type ClaimInviteMutationError = Error

/**
 * @summary Claim an invite to gain access to a checklist
 */
export const useClaimInvite = <TError = Error>(
  token: string, options?: { swr?:SWRMutationConfiguration<Awaited<ReturnType<typeof claimInvite>>, TError, Key, Arguments, Awaited<ReturnType<typeof claimInvite>>> & { swrKey?: string }, }
) => {
