...
>(({ className, variant, children, onOpenChange, ...props }, ref) => {
  ...
  const handleTouchEnd = () => {
    setIsSwiping(false)
    if (swipeOffset.x > threshold || swipeOffset.y < -threshold) {
      onOpenChange?.(false)
    }
  ...
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), "pb-8", className)}
    ...
    >
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-10 h-1 rounded-full bg-foreground/20 group-[.destructive]:bg-destructive-foreground/30" />
    </ToastPrimitives.Root>
  )
})
