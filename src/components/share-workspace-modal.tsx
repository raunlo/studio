...
import { useGetWorkspaceInvites, useCreateWorkspaceInvite } from '@/api/workspace/workspace';
import { copyToClipboard, mapWorkspaceInviteToDisplay, type InviteDisplayData } from '@/lib/invite-utils';
...
export function ShareWorkspaceModal({
  ...
  const { trigger: createInvite, isMutating: isCreating } = useCreateWorkspaceInvite(workspaceId);

  const displayInvites: InviteDisplayData[] = invites.map(mapWorkspaceInviteToDisplay);
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      if (result?.inviteUrl) {
        ...
        const tokenMatch = result.inviteUrl.match(/\/workspace-invites\/([^/]+)\/claim/);
        if (tokenMatch && tokenMatch[1]) {
          urlToCopy = `${window.location.origin}/workspace-invites/${tokenMatch[1]}/claim`;
        }
  ...
  const handleCopyLink = async (url: string) => {
    try {
      ...
      if (success) {
        toast({ title: t('share.linkCopied'), description: t('share.pasteToShare') });
      } else {
        toast({ title: t('share.copyFailed'), description: url, duration: 10000 });
      }
    } catch {
      toast({ title: t('share.copyFailed'), description: url, duration: 10000 });
    }
  ...
  const handleShare = async (url: string, name?: string | null) => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${t('workspaceInvite.title')}: ${workspaceName}`,
          text: name ? `${name} - ${t('workspaceInvite.shareMessage')}` : t('workspaceInvite.shareMessage'),
          url,
        });
          ...
          <div className="sticky top-0 z-10 border-b bg-background p-4 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-base sm:text-lg">
                {t('workspaceInvite.title')} &quot;{workspaceName}&quot;
              </DialogTitle>
              <DialogDescription className="text-xs sm:text-sm">
                {t('workspaceInvite.description')}
              </DialogDescription>
            </DialogHeader>
          </div>
            ...
            <div className="space-y-4">
              ...
              {isLoading ? (
                ...
                <div className="py-8 text-center text-sm text-muted-foreground">
                  {t('workspaceInvite.noActiveInvites')}
                </div>
              ) : (
                  ...
                  {displayInvites.map((invite) => {
                    ...
                    return (
                      <div key={invite.id} className="space-y-3 rounded-lg border p-3 sm:p-4">
                        <div className="space-y-2">
                          {inviteData?.name && (
                            <div className="text-sm font-medium sm:text-base">{inviteData.name}</div>
                          )}
                        ...
                        <div className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground sm:gap-2">
                          ...
                          {invite.claimedAt && (
                            <>
                              <span>•</span>
                              <span>{t('share.claimed')} {invite.claimedAt.toLocaleDateString()}</span>
                            </>
                          )}
                        </div>
